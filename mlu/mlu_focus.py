...
class FocusItem(BaseModel):
    id: str = Field(..., description="Unique identifier for the focus item")
    content: str = Field(..., description="Description of the focus")
    priority: float = Field(..., description="Priority score for this focus")
    is_global: bool = Field(
        ..., description="Whether this is a global focus or task-specific"
    )
    status: str = Field(
        ..., description="Current status: 'active', 'completed', 'pruned'"
    )
...
class MLUFocus:
    def __init__(
        ...
        self.focus_kms = KnowledgeManagementSystem(
            main_goal=f"Manage and prioritize focus areas for the MLU - Main goal: {main_role}",
            storage_goal=f"Store focus items that guide the learning process for the main role - Main goal: {main_role}",
            retrieval_goal=f"Retrieve relevant focus items to guide the current iteration and task - Main goal: {main_role}",
            chroma_db_path="./.db",
            compress_knowldge=compress_knowledge,
            collection_name=f"{collection_name}_focus" if collection_name else None,
        )

        self.current_global_focus = None
        self.current_task_focus = None
    ...
    def initialize_agents(self):
        class FocusUpdateOutput(BaseModel):
            global_focus: Optional[FocusItem] = Field(
                None, description="Updated global focus"
            )
            task_focus: Optional[FocusItem] = Field(
                None, description="Updated task-specific focus"
            )
            completed_focuses: List[str] = Field(
                [], description="IDs of completed focus items"
            )

        self.focus_update_agent = Agent(
            name="FocusUpdateAgent",
            role="Update and manage focus areas for the MLU",
            function=f"""
            Main Goal: {self.main_role}
            
            Your task is to update and manage focus areas for the MLU based on recent performance and feedback.
            Consider the main goal, current global and task-specific focuses, and recent feedback.
            
            Your responsibilities include:
            1. Updating the global focus if necessary
            2. Creating or updating a task-specific focus
            3. Identifying completed focus areas
            4. Ensuring focus areas align with the main goal and recent feedback
            
            Output:
            - An updated global focus (if changed)
            - An updated task-specific focus
            - A list of IDs for completed focus areas
            """,
            output_model=FocusUpdateOutput,
            disable_logging=disable_loging,
        )
    ...
    def call(
        ...
        **kwargs,
        ...
        # Retrieve relevant focus knowledge
        focus_knowledge = self.focus_kms.retrieve_knowledge(
            task, n_results=self.retrival_limit
        )
        focus_knowledge_str = (
            " ".join(
                [
                    item
                    for sublist in focus_knowledge["documents"]
                    for item in (sublist if isinstance(sublist, list) else [sublist])
                ]
            )
            if focus_knowledge["documents"]
            else "No relevant focus knowledge found."
        )

        # Include focus in prompt generation
        prompt_input = f"""
        Main Goal: {self.main_role}
        Task: {task}
        Relevant Prompt Knowledge: {prompt_knowledge_str}
        Current Global Focus: {self.current_global_focus}
        Current Task Focus: {self.current_task_focus}
        Relevant Focus Knowledge: {focus_knowledge_str}
        """
        generated_prompt = self.meta_prompt_agent(prompt_input).prompt
    ...
    def update_focus(self, task: str, feedback: FeedbackOutput):
        focus_update_input = f"""
        Main Goal: {self.main_role}
        Task: {task}
        Current Global Focus: {self.current_global_focus}
        Current Task Focus: {self.current_task_focus}
        Recent Feedback: {feedback.dict()}
        """
        focus_update = self.focus_update_agent(focus_update_input)

        if focus_update.global_focus:
            self.current_global_focus = focus_update.global_focus
            self.focus_kms.save_knowledge(self.current_global_focus.json())

        if focus_update.task_focus:
            self.current_task_focus = focus_update.task_focus
            self.focus_kms.save_knowledge(self.current_task_focus.json())

        # Prune completed focuses
        for focus_id in focus_update.completed_focuses:
            self.focus_kms.prune("Completed focus", [focus_id])
    ...
    def print_knowledge_base(self):
        ...
        focus_entries = self.focus_kms.collection.get()
        if not focus_entries["ids"]:
            console.print("[bold red]Focus knowledge base is empty.[/bold red]")
        else:
            for idx, doc in enumerate(focus_entries["documents"]):
                console.print(f"Entry {idx + 1}: {doc}")
